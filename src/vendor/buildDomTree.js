window.buildDomTree = (
  ...
  const MAGICBROWSE_DOM_NODE_ID_COUNTER_PROPERTY = '__mercuryoMagicBrowseDomNodeIdCounter';
  ...
  function highlightElement(element, index, parentIframe = null) {
    ...
    try {
      ...
      const backgroundColor = baseColor + '0D'; // 10% opacity version of the color
  ...
  function getElementPosition(currentElement) {
    ...
    const siblings = Array.from(parent.children).filter(
      sib => sib.id !== HIGHLIGHT_CONTAINER_ID && sib.nodeName.toLowerCase() === tagName,
    );
  ...
  function getXPathTree(element, stopAtBoundary = true) {
    ...
    while (currentElement && currentElement.nodeType === Node.ELEMENT_NODE) {
      ...
      // Stop after adding the local element segment, before crossing the boundary.
      if (
        stopAtBoundary &&
  ...
  function hasBooleanAttribute(element, name) {
    return element.hasAttribute(name) || element.getAttribute(name) === 'true' || element.getAttribute(name) === '';
  }

  function isDisabledByBrowserFacts(element) {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) return false;
    return (
      hasBooleanAttribute(element, 'disabled') ||
      element.getAttribute('aria-disabled') === 'true' ||
      hasBooleanAttribute(element, 'inert') ||
      element.inert === true ||
      hasBooleanAttribute(element, 'hidden') ||
  ...
  function isReadonlyTextControl(element) {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) return false;
    const tagName = element.tagName.toLowerCase();
    if (tagName !== 'input' && tagName !== 'textarea') return false;
    return (
      hasBooleanAttribute(element, 'readonly') ||
      element.getAttribute('aria-readonly') === 'true' ||
      element.readOnly === true
    );
  }

  function hasDirectReadonlyClickActivation(element) {
    ...
    if ((role && activationRoles.has(role)) || (ariaRole && activationRoles.has(ariaRole))) {
      return true;
    }
    ...
    try {
      ...
      if (typeof getEventListenersForNode === 'function') {
        ...
        const clickEvents = ['click', 'mousedown', 'mouseup', 'keydown', 'keyup'];
      ...
      // If checking listeners fails, rely on other direct facts.
    }
  ...
  function isInteractiveElement(element) {
    ...
    if (isDisabledByBrowserFacts(element)) {
      return false;
    }

    const isReadonlyText = isReadonlyTextControl(element);
    if (isReadonlyText && !hasDirectReadonlyClickActivation(element)) {
      return false;
    }
    ...
    // Genius fix for almost all interactive elements. For readonly text controls
    // cursor alone is not enough to turn the display field into an action target.
    if (isInteractiveCursor && !isReadonlyText) {
      return true;
    }
    ...
    if (interactiveElements.has(tagName)) {
      ...
      if (element.readOnly) {
        return hasDirectReadonlyClickActivation(element);
      }
  ...
  const OWNER_CONTEXT_ATTRIBUTE_NAMES = new Set(['aria-label', 'aria-labelledby', 'role', 'hidden', 'inert']);

  function isOwnerContextAttributeCarrier(element) {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) return false;
    const tagName = element.tagName.toLowerCase();
    const role = element.getAttribute('role')?.trim().toLowerCase();
    return OWNER_CONTEXT_TAG_NAMES.has(tagName) || Boolean(role && OWNER_CONTEXT_ROLES.has(role));
  }
  ...
  function isBodyLevelLayoutWrapper(element) {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) return false;
    if (!element.parentElement || !element.parentElement.isSameNode(document.body)) return false;

    const tagName = element.tagName.toLowerCase();
    if (DISTINCT_INTERACTIVE_TAGS.has(tagName)) return false;
    ...
    const coversMostViewport =
      rect.width >= window.innerWidth * 0.8 && rect.height >= window.innerHeight * 0.4;
    const hasPageLikeContent = element.children.length > 1 || (element.textContent || '').trim().length > 200;

    return coversMostViewport && hasPageLikeContent;
  }
  ...
  function handleHighlighting(nodeData, node, parentIframe, isParentHighlighted) {
    ...
    if (isBodyLevelLayoutWrapper(node)) {
      return false;
    }
  ...
  function getMagicBrowseDomNodeId(element) {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) {
      return null;
    }

    const existing = element[MAGICBROWSE_DOM_NODE_ID_PROPERTY];
    if (typeof existing === 'string' && existing.length > 0) {
      return existing;
    }

    const ownerWindow = element.ownerDocument?.defaultView || window;
    const nextId = Number(ownerWindow[MAGICBROWSE_DOM_NODE_ID_COUNTER_PROPERTY] || 0) + 1;
    ownerWindow[MAGICBROWSE_DOM_NODE_ID_COUNTER_PROPERTY] = nextId;
    const internalNodeId = `shadow-${nextId}`;

    try {
      Object.defineProperty(element, MAGICBROWSE_DOM_NODE_ID_PROPERTY, {
        value: internalNodeId,
        enumerable: false,
        configurable: false,
      });
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false, depth = 0, isInsideShadowTree = false) {
    ...
    if (node.shadowRoot || isInsideShadowTree) {
      const internalNodeId = getMagicBrowseDomNodeId(node);
      if (internalNodeId) {
        nodeData.internalNodeId = internalNodeId;
      }
    }

    // Get attributes for interactive elements or potential text containers
    const isInteractiveAttributeCarrier = isInteractiveCandidate(node);
    const isOwnerAttributeCarrier = isOwnerContextAttributeCarrier(node);
    if (
      isInteractiveAttributeCarrier ||
      isOwnerAttributeCarrier ||
      node.tagName.toLowerCase() === 'iframe' ||
      node.tagName.toLowerCase() === 'body'
    ) {
      const attributeNames = node.getAttributeNames?.() || [];
      for (const name of attributeNames) {
        if (!isInteractiveAttributeCarrier && isOwnerAttributeCarrier && !OWNER_CONTEXT_ATTRIBUTE_NAMES.has(name)) {
          continue;
        }
      ...
      if (node instanceof HTMLInputElement || node instanceof HTMLTextAreaElement) {
        ...
        if (node.readOnly) {
          nodeData.attributes.readonly = '';
        }
        if (node.disabled) {
          nodeData.attributes.disabled = '';
        }
    ...
    if (node.tagName) {
          ...
          try {
            ...
            if (iframeDoc && iframeDoc.childNodes) {
              for (const child of Array.from(iframeDoc.childNodes)) {
                const domElement = buildDomTree(child, node, false, depth + 1, false);
                if (domElement) nodeData.children.push(domElement);
              }
            }
          } catch (e) {
      ...
      // Handle rich text editors and contenteditable elements
        ...
        for (const child of Array.from(node.childNodes)) {
          const domElement = buildDomTree(child, parentIframe, nodeWasHighlighted, depth + 1, isInsideShadowTree);
          if (domElement) nodeData.children.push(domElement);
        }
      } else {
        // Handle shadow DOM
        if (node.shadowRoot) {
          ...
          for (const child of Array.from(node.shadowRoot.childNodes)) {
            const domElement = buildDomTree(child, parentIframe, nodeWasHighlighted, depth + 1, true);
            if (domElement) nodeData.children.push(domElement);
          }
        }
        ...
        for (const child of Array.from(node.childNodes)) {
          ...
          const domElement = buildDomTree(child, parentIframe, passHighlightStatusToChild, depth + 1, isInsideShadowTree);
          if (domElement) nodeData.children.push(domElement);
        }
      }
    }
