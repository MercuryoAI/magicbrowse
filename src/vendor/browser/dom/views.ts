import type { CoordinateSet, HashedDomElement, ViewportInfo } from './history/view.js';
import { HistoryTreeProcessor } from './history/service.js';
import { capTextLength } from '../util.js';
import { createLogger } from '../../../adapter/logger.js';

const logger = createLogger('DOMViews');
...
const CONTEXT_CONTROL_ATTRIBUTES = [
  'type',
  'name',
  'role',
  'value',
...
const BOOLEAN_CONTEXT_ATTRIBUTES = new Set([
  'disabled',
  'readonly',
  'aria-disabled',
  'aria-readonly',
...
const OWNER_CONTEXT_TAG_NAMES = new Set(['section', 'fieldset', 'dialog', 'table']);
const OWNER_CONTEXT_ROLES = new Set(['dialog', 'grid', 'listbox', 'table', 'section']);
const OWNER_CONTEXT_TEXT_LENGTH = 80;
...
function formatNodeAttributes(
  ...
  for (const [key, value] of Object.entries(node.attributes)) {
    ...
    if (normalized !== '') {
      attributesToInclude[key] = normalized;
    } else if (BOOLEAN_CONTEXT_ATTRIBUTES.has(key)) {
      attributesToInclude[key] = 'true';
    }
  }
...
function isPlannerVisibleControlContext(node: DOMElementNode): boolean {
  if (!node.isVisible) return false;
  const tagName = (node.tagName ?? '').toLowerCase();
  if (!['button', 'input', 'textarea', 'select', 'option'].includes(tagName)) return false;
  const disabled =
    hasBooleanAttributeValue(node.attributes.disabled) ||
    node.attributes['aria-disabled'] === 'true' ||
    hasBooleanAttributeValue(node.attributes.inert);
  const readonly =
    (tagName === 'input' || tagName === 'textarea') &&
    (hasBooleanAttributeValue(node.attributes.readonly) || node.attributes['aria-readonly'] === 'true');
  return disabled || readonly;
}

function ownerContextLineForNode(node: DOMElementNode): string | null {
  if (!isVisibleOwnerContextContainer(node) || node.highlightIndex !== null) {
    return null;
  }

  if (!hasActionableOrControlContextDescendant(node)) {
    return null;
  }

  const tagName = (node.tagName ?? '').toLowerCase();
  const renderedTagName = node.tagName ?? 'element';
  const role = node.attributes.role?.trim();
  const ariaLabel = normalizeOwnerContextText(node.attributes['aria-label']);
  const ariaLabelledBy = normalizeOwnerContextText(node.attributes['aria-labelledby']);
  const headingText = directOwnerHeadingText(node);

  if (!ariaLabel && !ariaLabelledBy && !headingText) {
    return null;
  }

  const attributes: string[] = [];

  if (role && role.toLowerCase() !== tagName) {
    attributes.push(`role=${formatOwnerContextValue(role)}`);
  }
  if (ariaLabel) {
    attributes.push(`aria-label=${formatOwnerContextValue(ariaLabel)}`);
  } else if (ariaLabelledBy) {
    attributes.push(`aria-labelledby=${formatOwnerContextValue(ariaLabelledBy)}`);
  }

  const attributesText = attributes.length > 0 ? ` ${attributes.join(' ')}` : '';
  const text = !ariaLabel && !ariaLabelledBy && headingText ? `>${formatOwnerContextValue(headingText)}` : '>';
  return `<${renderedTagName}${attributesText}${text}`;
}

function isVisibleOwnerContextContainer(node: DOMElementNode): boolean {
  if (hasBooleanAttributeValue(node.attributes.hidden) || hasBooleanAttributeValue(node.attributes.inert)) {
    return false;
  }

  const tagName = (node.tagName ?? '').toLowerCase();
  const role = node.attributes.role?.trim().toLowerCase();
  return OWNER_CONTEXT_TAG_NAMES.has(tagName) || (role ? OWNER_CONTEXT_ROLES.has(role) : false);
}

function hasActionableOrControlContextDescendant(node: DOMElementNode): boolean {
  for (const child of node.children) {
    if (!(child instanceof DOMElementNode)) {
      continue;
    }
    if ((child.isVisible && child.highlightIndex !== null) || isPlannerVisibleControlContext(child)) {
      return true;
    }
    if (hasActionableOrControlContextDescendant(child)) {
      return true;
    }
  }
  return false;
...
function directOwnerHeadingText(node: DOMElementNode): string | null {
  if ((node.tagName ?? '').toLowerCase() === 'fieldset') {
    const legend = node.children.find(
      (child): child is DOMElementNode =>
        child instanceof DOMElementNode && (child.tagName ?? '').toLowerCase() === 'legend' && child.isVisible,
    );
    const legendText = legend ? normalizeOwnerContextText(legend.getAllTextTillNextClickableElement()) : null;
    if (legendText) {
      return legendText;
    }
  }

  const heading = node.children.find((child): child is DOMElementNode => {
    if (!(child instanceof DOMElementNode) || !child.isVisible) {
      return false;
    }
    return /^h[1-6]$/.test((child.tagName ?? '').toLowerCase());
  });
  return heading ? normalizeOwnerContextText(heading.getAllTextTillNextClickableElement()) : null;
}

function normalizeOwnerContextText(value: string | undefined): string | null {
  const normalized = String(value ?? '').replace(/\s+/g, ' ').trim();
  return normalized === '' ? null : normalized;
}

function formatOwnerContextValue(value: string): string {
  return capTextLength(value, OWNER_CONTEXT_TEXT_LENGTH);
}

function hasBooleanAttributeValue(value: string | undefined): boolean {
...
export class DOMElementNode extends DOMBaseNode {
  ...
  constructor(params: {
    ...
    this.internalNodeId = params.internalNodeId ?? null;
    ...
    const processNode = (node: DOMBaseNode, depth: number): void => {
      ...
      if (node instanceof DOMElementNode) {
        ...
        if (node.highlightIndex !== null) {
          ...
          const attributesHtmlStr = formatNodeAttributes(node, includeAttributes, text);
        ...
        } else if (isPlannerVisibleControlContext(node)) {
          const text = node.getAllTextTillNextClickableElement();
          const attributesHtmlStr = formatNodeAttributes(node, CONTEXT_CONTROL_ATTRIBUTES, text);
          ...
          const ownerContextLine = ownerContextLineForNode(node);
          if (ownerContextLine) {
            formattedText.push(`${depthStr}${ownerContextLine}`);
            nextDepth += 1;
          }
        }
