...
import { guardrails } from '../../services/guardrails/index.js';
import { ResponseParseError } from '../agents/errors.js';
 ...
 * @param strict - If true, uses strict mode in guardrails (default: false)
 * @returns Filtered content string with malicious content removed
 */
export function filterExternalContent(rawContent: string | undefined, strict: boolean = false): string {
...
export function filterExternalContentWithReport(rawContent: string | undefined, strict: boolean = false) {
  if (!rawContent || rawContent.trim() === '') {
