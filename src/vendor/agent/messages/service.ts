...
import { MessageHistory, MessageMetadata } from './views.js';
...
export const MESSAGE_MANAGER_SNAPSHOT_VERSION = 1 as const;
type MessageContent = BaseMessage['content'];

interface NavigatorContinuePlanView {
...
interface PlannerTerminalOutcomeHistory {
  readonly status: 'completed' | 'blocked' | 'needs_handoff' | 'needs_approval';
  readonly observation: string;
  readonly challenges: string;
  readonly final_answer: string;
}

export type MessageManagerSerializedRole = 'system' | 'human' | 'ai' | 'tool';

export interface MessageManagerSerializedMessage {
  readonly role: MessageManagerSerializedRole;
...
export interface MessageManagerSnapshot {
  readonly version: typeof MESSAGE_MANAGER_SNAPSHOT_VERSION;
  readonly toolId: number;
  readonly totalTokens: number;
  readonly messages: readonly MessageManagerSerializedMessage[];
}
...
export default class MessageManager {
  ...
  private static taskInstructions(task: string): HumanMessage {
    ...
    const content = `Your delegated browser task is: """${cleanedTask}""". If the delegated browser task appears satisfied, stop browser work and use the done action in the next step so the planner can validate completion. If not, continue browser work within this delegated task.`;
  ...
  public snapshot(): MessageManagerSnapshot {
    return {
      version: MESSAGE_MANAGER_SNAPSHOT_VERSION,
      toolId: this.toolId,
      totalTokens: this.history.totalTokens,
      messages: this.history.messages.map(({ message, metadata }) => ({
        role: getSerializedMessageRole(message),
        type: getSerializedMessageType(message),
        content: cloneMessageContent(message.content),
        ...(message instanceof AIMessage && message.tool_calls
          ? { toolCalls: cloneJsonValue(message.tool_calls) }
          : {}),
  ...
  public restoreSnapshot(snapshot: MessageManagerSnapshot): void {
    if (snapshot.version !== MESSAGE_MANAGER_SNAPSHOT_VERSION) {
      throw new Error(`Unsupported MessageManager snapshot version ${String(snapshot.version)}.`);
    }

    const history = new MessageHistory();
    for (const serialized of snapshot.messages) {
      history.addMessage(
        restoreSerializedMessage(serialized),
        new MessageMetadata(serialized.metadata.tokens, serialized.metadata.message_type),
      );
    }

    this.history = history;
    this.toolId = Math.max(snapshot.toolId, nextToolIdFromMessages(snapshot.messages));
  }

  // Adapt: expose total token count for diagnostic events.
  ...
  public addNewTask(newTask: string): void {
    ...
    // Filter and wrap user text
    const cleanedTask = filterExternalContent(userText);
    const content = `Your new delegated browser task is: """${cleanedTask}""". This is a follow-up of the previous tasks. Use previous context as task history and finish the new delegated browser task.`;
    const wrappedUser = wrapUserRequest(content, false);
  ...
  public addPlan(plan?: string, position?: number): void {
    ...
    const navigatorPlan = toNavigatorContinuePlanView(plan);
    if (navigatorPlan) {
      const msg = new AIMessage({ content: `<plan>${JSON.stringify(navigatorPlan)}</plan>` });
      this.addMessageWithTokens(msg, null, position);
      return;
    }

    const terminalOutcome = toPlannerTerminalOutcomeHistory(plan);
    if (terminalOutcome) {
      const terminalContext = JSON.stringify(terminalOutcome);
      const msg = new AIMessage({
        content:
          'Task-history context only. This is not browser-action guidance.\n' +
          `<planner_outcome_context>${terminalContext}</planner_outcome_context>`,
      });
      this.addMessageWithTokens(msg, null, position);
    }
  }
  ...
  public getMessages(): BaseMessage[] {
    const messages = this.history.messages
      .filter(m => {
        if (!m.message) {
          logger.error('Filtering out message with undefined message property:', m);
          return false;
        }
        return true;
      })
    ...
    for (const m of this.history.messages) {
      ...
      if (m.message) {
        ...
        logger.error('Found message with undefined message property:', m);
...
function getSerializedMessageRole(message: BaseMessage): MessageManagerSerializedRole {
...
function getSerializedMessageType(message: BaseMessage): string {
  const typedMessage = message as BaseMessage & { _getType?: () => string };
  return typeof typedMessage._getType === 'function' ? typedMessage._getType() : getSerializedMessageRole(message);
}

function restoreSerializedMessage(serialized: MessageManagerSerializedMessage): BaseMessage {
  switch (serialized.role) {
    case 'system':
      return new SystemMessage({ content: cloneMessageContent(serialized.content) });
    case 'human':
      return new HumanMessage({ content: cloneMessageContent(serialized.content) });
    case 'ai':
      return new AIMessage({
        content: cloneMessageContent(serialized.content),
        ...(serialized.toolCalls ? { tool_calls: cloneJsonValue(serialized.toolCalls) as ToolCall[] } : {}),
      });
    case 'tool':
      if (!serialized.toolCallId) {
        throw new Error('Serialized tool message is missing tool_call_id.');
      }
...
function cloneMessageContent(content: MessageContent): MessageContent {
  return typeof content === 'string' ? content : cloneJsonValue(content);
}
...
function nextToolIdFromMessages(messages: readonly MessageManagerSerializedMessage[]): number {
  let maxToolId = 0;
  for (const message of messages) {
    if (message.toolCallId) {
      maxToolId = Math.max(maxToolId, Number.parseInt(message.toolCallId, 10) || 0);
    }
    for (const toolCall of message.toolCalls ?? []) {
      if (toolCall.id) {
        maxToolId = Math.max(maxToolId, Number.parseInt(toolCall.id, 10) || 0);
      }
    }
  }
  return maxToolId + 1;
...
function toNavigatorContinuePlanView(plan: string): NavigatorContinuePlanView | null {
  let parsed: unknown;
  try {
  ...
  if (!isRecord(parsed) || parsed.status !== 'continue') {
    return null;
  }

  const observation = stringField(parsed, 'observation');
  const challenges = stringField(parsed, 'challenges');
  const nextSteps = stringField(parsed, 'next_steps');
  if (observation === null || challenges === null || nextSteps === null) {
    return null;
  }

  const view = {
    status: 'continue',
    observation: filterExternalContent(observation, false),
    challenges: filterExternalContent(challenges, false),
    next_steps: filterExternalContent(nextSteps, false),
  } satisfies NavigatorContinuePlanView;

  return view.next_steps.trim().length > 0 ? view : null;
}

function toPlannerTerminalOutcomeHistory(plan: string): PlannerTerminalOutcomeHistory | null {
  let parsed: unknown;
  try {
    parsed = JSON.parse(plan);
  } catch {
  ...
  const observation = stringField(parsed, 'observation');
  const challenges = stringField(parsed, 'challenges');
  const finalAnswer = stringField(parsed, 'final_answer');
  if (observation === null || challenges === null || finalAnswer === null) {
    return null;
  }
  ...
  const view = {
    ...
    observation: filterExternalContent(observation, false),
    challenges: filterExternalContent(challenges, false),
    final_answer: filterExternalContent(finalAnswer, false),
  } satisfies PlannerTerminalOutcomeHistory;

  return view.final_answer.trim().length > 0 ? view : null;
}

function isTerminalStatus(value: unknown): value is PlannerTerminalOutcomeHistory['status'] {
  return (
    value === 'completed' ||
    value === 'blocked' ||
    value === 'needs_handoff' ||
