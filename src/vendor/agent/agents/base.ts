...
import type { BasePrompt } from '../prompts/base.js';
...
import { convertInputMessages, extractJsonFromModelOutput, removeThinkTags } from '../messages/utils.js';
import { isAbortedError, ResponseParseError } from './errors.js';
import { ProviderTypeEnum } from '../../../adapter/storage-stubs.js';
...
export abstract class BaseAgent<T extends z.ZodType, M = unknown> {
  ...
  async invoke(inputMessages: BaseMessage[]): Promise<this['ModelOutput']> {
    ...
    try {
      ...
      if (isDebug()) {
        debugJson(`[llm-output.raw] agent=${this.id} model=${this.modelName}`, response);
      }
      ...
      if (typeof response.content === 'string') {
        ...
        if (parsed) {
          if (isDebug()) {
            debugJson(`[llm-output.parsed] agent=${this.id} model=${this.modelName}`, parsed);
          }
          return parsed;
        }
      }
