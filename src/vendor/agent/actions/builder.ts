import { ActionResult, type AgentContext } from '../types.js';
...
import { ExecutionState, Actors } from '../event/types.js';
import type { BaseChatModel } from '@langchain/core/language_models/chat_models';
import { wrapUntrustedContent } from '../messages/utils.js';
import type { DOMElementNode } from '../../browser/dom/views.js';
import type { InputTargetActivationDiagnostics } from '../../browser/page.js';

const logger = createLogger('Action');
const ACTION_VALUE_PREVIEW_LENGTH = 160;
...
export class ActionBuilder {
  ...
  buildDefaultActions() {
    ...
    const done = new Action(async (input: z.infer<typeof doneActionSchema.schema>) => {
      const metadata = { actionName: doneActionSchema.name, kind: 'navigator.done' as const };
      this.context.emitEvent(Actors.NAVIGATOR, ExecutionState.ACT_START, doneActionSchema.name, metadata);
      this.context.emitEvent(Actors.NAVIGATOR, ExecutionState.ACT_OK, input.text, metadata);
    ...
    const clickElement = new Action(
      async (input: z.infer<typeof clickElementActionSchema.schema>) => {
        ...
        try {
          ...
          const newTabId = await waitForNewTabId(
            () => this.context.browserContext.getAllTabIds(),
            initialTabIds,
            isExplicitNewTabTarget(elementNode) ? { timeoutMs: 1000, intervalMs: 50 } : { timeoutMs: 0, intervalMs: 50 },
          );
          if (newTabId !== null) {
            const newTabMsg = `${t('act_click_newTabOpened')} (tab ${newTabId})`;
            msg += ` - ${newTabMsg}`;
    ...
    const inputText = new Action(
      async (input: z.infer<typeof inputTextActionSchema.schema>) => {
        ...
        const inputResult = await page.inputTextElementNode(this.context.options.useVision, elementNode, input.text);
        const postActionState = await page.getState();
        const msg = buildInputTextActionEvidence({
          index: input.index,
          attemptedValue: input.text,
          observedElement: elementNode,
          postActionElement: postActionState?.selectorMap.get(input.index) ?? null,
        ...
        try {
          ...
          if (options && options.length > 0) {
            ...
            const formattedOptions: string[] = options.map(opt => {
              // Encoding ensures AI can use exact strings in select_dropdown_option.
              const encodedText = JSON.stringify(opt.text);
              const encodedValue = JSON.stringify(opt.value);
              return `${opt.index}: text=${encodedText} value=${encodedValue}`;
            });
    ...
    const selectDropdownOption = new Action(
      async (input: z.infer<typeof selectDropdownOptionActionSchema.schema>) => {
        ...
        logger.debug(`Attempting to select '${input.text}' by ${input.matchBy} using xpath: ${elementNode.xpath}`);

        try {
          const result =
            input.matchBy === 'value'
              ? await page.selectDropdownOptionByValue(input.index, input.text)
...
function buildInputTextActionEvidence(input: {
  readonly index: number;
  readonly attemptedValue: string;
  readonly observedElement: DOMElementNode;
  readonly postActionElement: DOMElementNode | null;
  readonly inputTarget: InputTargetActivationDiagnostics;
}): string {
  const lines = [
    `Input text action ran for index ${input.index}.`,
    `Attempted value: ${formatActionValuePreview(input.attemptedValue)}.`,
  ];

  if (!input.postActionElement) {
    appendPageChangedAfterInputEvidence(lines);
    return lines.join(' ');
  }

  if (
    input.inputTarget.source === 'proxy_active_target' ||
    !sameInputEvidenceTarget(input.observedElement, input.postActionElement)
  ) {
    appendPageChangedAfterInputEvidence(lines);
    appendElementValidationEvidence(lines, input.postActionElement);
    return lines.join(' ');
  }

  appendElementValueEvidence(lines, input.postActionElement, input.attemptedValue);
  appendElementValidationEvidence(lines, input.postActionElement);
  return lines.join(' ');
}

function appendPageChangedAfterInputEvidence(lines: string[]): void {
  lines.push(
    'The page changed after input; use the current browser state and current indexes if another action is needed.',
  );
}

function sameInputEvidenceTarget(left: DOMElementNode, right: DOMElementNode): boolean {
  if (normalizeInputEvidenceString(left.tagName) !== normalizeInputEvidenceString(right.tagName)) {
    return false;
  }
  if (normalizeInputEvidenceString(left.xpath) !== normalizeInputEvidenceString(right.xpath)) {
    return false;
  }

  for (const name of ['id', 'name', 'type', 'role', 'autocomplete', 'placeholder']) {
    if ((left.attributes[name] ?? null) !== (right.attributes[name] ?? null)) {
      return false;
    }
  }
...
function appendElementValueEvidence(lines: string[], element: DOMElementNode, attemptedValue: string): void {
  const value = element.attributes.value;
  if (value === undefined) {
    lines.push('Current field value is not readable from the current target.');
    return;
  }
  if (value === attemptedValue) {
    lines.push('Current field value matches the attempted text.');
    return;
  }
  lines.push('Current field value differs from the attempted text.');
}

function appendElementValidationEvidence(lines: string[], element: DOMElementNode): void {
  const evidence = [
    ['aria-invalid', element.attributes['aria-invalid']],
    ['data-state', element.attributes['data-state']],
  ...
  lines.push(`Post-action validation/state facts: ${evidence.map(([name, value]) => `${name}=${JSON.stringify(value)}`).join(', ')}.`);
}

function normalizeInputEvidenceString(value: string | null | undefined): string | null {
  const trimmed = value?.trim();
  return trimmed ? trimmed.toLowerCase() : null;
}

function formatActionValuePreview(value: string): string {
  if (value.length <= ACTION_VALUE_PREVIEW_LENGTH) {
    return JSON.stringify(value);
  }

  return `${value.length} characters, starts with ${JSON.stringify(`${value.slice(0, ACTION_VALUE_PREVIEW_LENGTH)}...`)}`;
}

function isExplicitNewTabTarget(element: DOMElementNode): boolean {
  return element.attributes.target?.trim().toLowerCase() === '_blank';
}

async function waitForNewTabId(
  getAllTabIds: () => Promise<Set<number>>,
  initialTabIds: ReadonlySet<number>,
  ...
  while (true) {
    const currentTabIds = await getAllTabIds();
