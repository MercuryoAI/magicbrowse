...
export const selectDropdownOptionActionSchema: ActionSchema = {
  ...
  description:
    'Select dropdown option for interactive element index by exact visible text or exact option value',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    index: z.number().int().describe('index of the dropdown element'),
    text: z.string().describe('text or value of the option'),
    matchBy: z.enum(['text', 'value']).default('text').describe('match option by exact text or exact value'),
  }),
};
